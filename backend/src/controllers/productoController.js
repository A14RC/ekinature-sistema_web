...
const productoController = {
    ...
    obtenerProductoPorId: async (req, res) => {
        try {
            ...
            if (producto.length === 0) return res.status(404).json({ mensaje: 'Producto no encontrado' });
            res.json(producto[0]);
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    ...
    actualizarProducto: async (req, res) => {
        try {
            ...
            if (productoActual.length === 0) return res.status(404).json({ mensaje: 'Producto no encontrado' });
